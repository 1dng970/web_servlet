...
<%
   ...
   if(views == null){   // 데이터가 없을 경우 script 발동 후 list 페이지로 이동
...
%>
<span class="n">첨부파일 : <div class="data1"><a href="../notice_file/<%=views.get(5)%>" target="_blank"><%=views.get(5)%></div></span><br>
<%
   }
%>
<br><br>
...
패스워드 : <inpyt type="password" name="npw">
</form>
...
function notice_info(p){
   ...
   case 3:
      if(confirm("해당 게시물을 삭제시 복구되지 않습니다.")){
    	  if(frm.npw.value==""){
    		  alert("게시물을 삭제시 패스워드를 입력하셔야 합니다.");
    	  }else{
    		  frm.submit();
